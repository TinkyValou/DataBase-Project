...
\begin{document}
    ...
    \section{Conclusion}
        Ce projet nous aura permis de nous rendre compte de ce à quoi pourrait ressembler une réelle Base de Donnée. Que ce soit la rechercher préliminaire des dépendances fonctionnelles de nos différents attributs, à l'implémentation des objets permettant la maintenance de l'intégrité de notre BD, en passant par la mise en contexte de notre projet, ce dernier nous a faire prendre conscience du recul et de la rigeur qu'il faut avoir vis à vis du problème rencontré pour créer un shéma correct associé.\smallskip
        
        \noindent La Base de Donnée que nous avons créé est bien entendu incomplète. Il nous reste encore de nombreux cas à gérer comme par exemple eviter qu'un Client n'apporte de modification à quelqu'un d'autre que lui-même (il en va de même pour les Restaurants et les Coursiers), ou encore créer une fonction qui nous retourne le nombre de courses qu'un Coursier à effuctuer sur une periode entrée en paramètre.\smallskip
  
        \noindent Cependant, elle reste correcte et permet de répondre aux besoins essentiels d'un service de livraison de nourriture. Les rôles permettent d'assurer son intégrité, les triggers, fonctions/procédures et view une certaine fonctionnabilité. De plus, après deux itération de notre algorithme de décompostion, nous avons finis par trouver un modèle relationnel qui nous correspondait.\smallskip

        \noindent Pour conclure, ce projet nous aura permis de bien saisir le module de Base de Donnée 2 dans son entièreté tout en se rendant compte des différentes 
\end{document}